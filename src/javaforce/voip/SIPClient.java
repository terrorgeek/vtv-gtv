...
public class SIPClient extends SIP implements SIPInterface {
   ...
   * @param iface                 must be a SIPClientInterface where SIP events are dispatched to.<br>
   */
   ...
   * @return : if message was sent to server successfully<br>
   *         This function does not block waiting for a reply.
   *         You should receive onRegister() thru the SIPClientInterface when a reply is returned from server.<br>
   *         Password may be "." which causes getResponse() thru the SIPClientInterface to be called for password verification,
   *         so you don't have to send password to SIPClient.<br>
   *         This is useful if the client app communicates to the SIPClient code remotely (as is the case with RTMP2SIP).<br>
   */
   ...
   * @return : if message was sent to server<br>
   *         This function does not block waiting for a reply.
   *         You should receive either registered() or unauthorized() thru the SIPClientInterface when a reply is returned from server.<br>
   */
  ...
  public boolean register(String user, String auth, String pass, int expires) {
    ...
    cd.src.contact = "<sip:" + user + "@" + getlocalhost() + ":" + localport + ">";
    cd.src.uri = "sip:" + remotehost;  // + ";rinstance=" + getrinstance();
    cd.src.branch = getbranch();
    cd.src.cseq++;
    if ((pass == null) || (pass.length() == 0)) return true;  //non-register mode
    cd.authsent = false;
  ...
  /**
   * Send an empty SIP message to server.  This should be done periodically to keep firewalls open.  Most routers close UDP connections after 60 seconds.
   */
  ...
  private boolean isServerOnPrivateNetwork() {
    ...
    if (remoteip.startsWith("10.")) return true;
//    if (remoteip.startsWith("172.[16-31].")) return true;  //who uses that?
  ...
  /**
   * Determines local IP address.  Uses several methods (such as http://checkip.dyndns.org).
   */
      ...
      for (Enumeration<NetworkInterface> en = NetworkInterface.getNetworkInterfaces(); en.hasMoreElements(); ) {
        ...
        for (Enumeration<InetAddress> enumIpAddr = networkInterface.getInetAddresses(); enumIpAddr.hasMoreElements(); ) {
          ...
          if (!inetAddress.isLoopbackAddress() && InetAddressUtils.isIPv4Address(inetAddress.getHostAddress())
              && (inetAddress.getHostAddress().startsWith("192.168.") || inetAddress.getHostAddress().startsWith("10."))) {
  ...
  private boolean issue(CallDetails cd, String cmd, String extra, boolean sdp, boolean src) {
    ...
    if (extra != null) {  //NOTE:refer has special headers and auth may need to add to them : BUG:this causes a build up of redudant headers sometimes
      if (cdsd.extra != null) cdsd.extra += extra;
      else cdsd.extra = extra;
    }
    cd.dst.host = remoteip;
    cd.dst.port = remoteport;
    StringBuffer req = new StringBuffer();
    req.append(cmd + " " + cdsd.uri + " SIP/2.0\r\n");
    req.append("Via: SIP/2.0/UDP " + getlocalhost() + ":" + localport + ";branch=" + cdsd.branch + ";rport\r\n");
    ...
    if (cdsd.extra != null) req.append(cdsd.extra);
    if ((cd.sdp != null) && (sdp)) {
  ...
  public String invite(String to, int rtp_port_audio, int rtp_port_video, Codec codecs[]) {
    ...
    cd.src.cseq++;
    cd.authsent = false;
    if (!issue(cd, "INVITE", null, true, true)) return null;
    return callid;
  }
  ...
  /**
   * Sends a reINVITE to server with a special SDP packet to place the call on hold.
   */
...
//    cd.src.cseq++;  //do NOT inc on hold request
    cd.authsent = false;
    if (!issue(cd, "INVITE", null, true, true)) return false;
    return true;
  }

  /**
   * Sends a reINVITE to server with a special SDP packet to remove the hold on a call or change other SDP details.
   */
...
//    for(int a=0;a<codecs.length;a++) JFLog.log("reinvite : codecs[]=" + codecs[a].name + ":" + codecs[a].id);
  ...
  public boolean bye(String callid) {
    ...
    cd.src.cseq++;
    cd.authsent = false;
    cd.src.uri = "sip:" + cd.src.to[1] + "@" + remotehost + ":" + remoteport;
  ...
  public boolean accept(String callid, int rtp_port_audio, int rtp_port_video, Codec codecs[]) {
    CallDetails cd = getCallDetails(callid);
    //cd.dst.from = replacetag(cd.dst.from, generatetag());  //assign tag
    cd.src.codecs = codecs;
    cd.src.rtp_port_audio = rtp_port_audio;
    cd.src.rtp_port_video = rtp_port_video;
    buildsdp(cd, cd.src);
    reply(cd, "INVITE", 200, "OK", true, false);
    //need to swap to/from for BYE/CANCEL
  ...
  public void packet(String msg[], String remoteip, int remoteport) {
    try {
      ...
      if (tmp == null) tmp = getHeader("t:", msg);
      cd.dst.to = split(tmp);
      //get cd.dst.from
      tmp = getHeader("From:", msg);
      if (tmp == null) tmp = getHeader("f:", msg);
      cd.dst.from = split(tmp);
      //get via list
      cd.dst.vialist = getvialist(msg);
      //set contact
      cd.dst.contact = "<sip:" + user + "@" + getlocalhost() + ":" + localport + ">";
      //get uri (it must equal the Contact field)
      cd.dst.uri = getHeader("Contact:", msg);
      if (cd.dst.uri == null) cd.dst.uri = getHeader("m:", msg);
      if (cd.dst.uri != null) {
...
//            setCallDetails(callid, null);
        ...
        case 200:
          ...
          break;
        case 202:
          if (cmd.equals("REFER")) iface.onRefer(this, callid);
          break;
        case 401:
          if (cmd.equals("REGISTER")) {
            if (cd.authsent) {
              ...
              epass = getAuthResponse(cd.authstr, auth, pass, remotehost, cmd, "Authorization:");
              if (epass == null) {
        ...
        case 407:
          ...
          if (cd.authsent) {
            ...
            epass = getAuthResponse(cd.authstr, auth, pass, remotehost, cmd, "Proxy-Authorization:");
            if (epass == null) {
              JFLog.log("err:gen auth failed");
              break;
            }
