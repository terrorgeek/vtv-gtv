...
public class RTPChannel {
  ...
  public void writeRTP(byte data[], int off, int len) {
    if (!rtp.active) return;
    if (rtp.remoteport == -1) return;  //not ready yet (NATing)
  ...
  public void writeDTMF(char digit, boolean end) {
    ...
    buildHeader(data, rtp.CODEC_RFC2833.id, getseqnum(), gettimestamp(20), getssrc());
    switch (digit) {
      case '*': data[12] = 10; break;
      case '#': data[12] = 11; break;
      default: data[12] = (byte)(digit - '0'); break;
    }
    if (end)
  ...
  public void buildHeader(byte data[], int id, int seqnum, int timestamp, int ssrc) {
    //build RTP header
    data[0] = (byte)0x80;  //version
    data[1] = (byte)id;    //0=g711u 18=g729a 34=H.263 etc.
