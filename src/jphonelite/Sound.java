...
//          playing = false;  //we are still playing
    ...
    if (at == null) {
      atStream = isRinging ? AudioManager.STREAM_RING : (Settings.current.speakerMode ? AudioManager.STREAM_MUSIC : AudioManager.STREAM_MUSIC);
      ...
      if (((atStream != AudioManager.STREAM_RING) && (isRinging)) || ((atStream == AudioManager.STREAM_RING) && (!isRinging))) {
        ...
        at.release();
        at = null;
        atStream = isRinging ? AudioManager.STREAM_RING : AudioManager.STREAM_MUSIC;
        if (!createAudioTrack()) return;
...
//          Log.i(TAG, "playing = false");
      ...
      if ((cc > 1) && (line != -1) && (lines[line].cnf)) {
        ...
        if ((line != -1) && (lines[line].talking) && (!lines[line].hld)) {
          ...
          else Log.i(TAG, "WARNING : rtp.getSamples() was empty.");
          if (inRinging) mix(mixed, getCallWaiting());
          isRinging = false;
          wlistAdd(mixed);
        } else {
...
//Log.i(TAG, "Sound.process(3) : " + System.currentTimeMillis() );
            if (line == a)
              ...
              encoded = lines[a].rtp.coder.encode(silence, 0, 160);
          }
    ...
    public void run() {
      ...
      try {
        while (recording) {
          ...
          Log.i(TAG, "Waiting for the remote client to connect");
          socket = serverSocket.accept();
          Log.i(TAG, "Remote client connected");
          byte[] audio = new byte[320];
          InputStream inputStream = socket.getInputStream();

//        AudioTrack audioTrack = new AudioTrack(AudioManager.STREAM_VOICE_CALL, 8000, AudioFormat.CHANNEL_OUT_MONO, AudioFormat.ENCODING_PCM_16BIT, 4096, AudioTrack.MODE_STREAM);
//        audioTrack.play();

          while (recording && inputStream.read(audio) != -1) {
            buffer = new short[160];
            ByteBuffer.wrap(audio).order(ByteOrder.LITTLE_ENDIAN).asShortBuffer().get(buffer);
            short[] outBuff = new short[160];
            System.arraycopy(buffer, 0, outBuff, 0, 160);
            if (read(outBuff)) {
              rlist.add(outBuff);
            }
//          audioTrack.write(outBuff, 0, 160);
          }
          socket.close();
          serverSocket.close();
//          audioTrack.stop();
//          audioTrack.release();
        ...
        Log.e(TAG, "Server failed. Input/output exception", e);
      } finally {
        Log.i(TAG, "Closing connection with remote client");
        try {
          if (socket != null) {
            socket.close();
          }
