...
public class Engine implements SIPClientInterface, RTPInterface {
  ...
  private VTVMainActivity main;

  private Engine() {}

  public synchronized static Engine getInstance(Context ctx, VTVMainActivity main) {
    if (instance == null) {
...
//          Log.i(TAG, "DTMF:" + digit);
  ...
  public void selectLine(int newline) {
    ...
    if (main != null) main.updateScreen();
  }
...
//      Log.i(TAG, "registering:" + host + ":" + port);
      try {
        ...
        lines[a].sip = null;
        if (a == line) if (main != null) main.updateScreen();
      }
    }
    for(int a=1;a<6;a++) {
    ...
    for(int a=0;a<6;a++) {
      if ((lines[a].incall)&&(!lines[a].trying)) {
        if (lines[a].callid.equals(callid)) {
          ...
          if (line == a) if (main != null) main.updateScreen();
        }
      }
    }
  }
